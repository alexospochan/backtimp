...
router.put("/:id/comentario-leido", async (req, res) => {
  try {
    ...
    // Buscar proyecto para validar jefe de cuadrilla
    const proyecto = await Proyecto.findById(reporte.proyectoId);
    if (!proyecto) {
      return res.status(404).json({ message: "Proyecto no encontrado" });
    }

    // Validar que userId esté en el array de jefesCuadrillaIds
    const esJefe = proyecto.jefesCuadrillaIds.some(
      (jefeId) => jefeId.toString() === userId
    );
    ...
    // Agregar userId a leidoPor si no está
    if (!reporte.leidoPor.some(id => id.toString() === userId)) {
